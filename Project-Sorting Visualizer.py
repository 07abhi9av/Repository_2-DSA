...
import user_def_module as mod_1


mod_1.signature()  # User Defined Module is imported
...
# Creating Functions for Sorting
...
def generate():
    ...
    barstart = 5
    barend = 15
    barList = []  # Empty list is created
    lengthList = []  # Empty list is created
...
window = Tk()
window.title('Sorting Visualizer - By Abhinav Aryan')
...
shuffle = tk.Button(window, text='Shuffle', command=generate, bg="yellow", fg="black", width=10, height=1)
exit_button = tk.Button(window, text="Close", command=window.destroy, bg="red", width=10, height=1)

# Setting the Buttons Position
exit_button.grid(column=45, row=1)
